...
def get_lseg_feat(
    ...
    if long_size <= crop_size:
        ...
        with torch.cuda.device_of(image):
            with torch.no_grad():
                outputs = image.new().resize_(batch, model.out_c, ph, pw).zero_().to("cpu")
                logits_outputs = image.new().resize_(batch, len(labels), ph, pw).zero_().to("cpu")
            count_norm = image.new().resize_(batch, 1, ph, pw).zero_().to("cpu")
        # grid evaluation
